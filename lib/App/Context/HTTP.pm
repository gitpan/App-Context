...
#############################################################################
## $Id: HTTP.pm 9944 2007-09-13 20:05:34Z spadkins $
#############################################################################

package App::Context::HTTP;
$VERSION = (q$Revision: 9944 $ =~ /(\d[\d\.]*)/)[0];  # VERSION numbers generated by svn
    ...
    eval {
        ...
        while ($#$events > -1) {
            ...
            if ($authorization->is_authorized("/App/$service_type/$service_name/$method", $user)) {
                $results = $self->call($service_type, $service_name, $method, $args);
